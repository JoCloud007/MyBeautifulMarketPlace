...
**2026-08-11 (discovery):** Client environment — web container logs empty, port 5192 unreachable. Root cause: `serve` (static file server) was NOT in any `package.json`. The Dockerfile used `CMD ["npx", "serve", ...]` which downloads `serve` on-the-fly via npm. On macOS with internet this works; in an air-gapped client container, `npx` cannot download → container starts but no `serve` process runs. Fix: added `serve` as an explicit `devDependency` in root `package.json` so it is installed by `npm install` on the host and copied into the image via `COPY node_modules`. Updated README troubleshooting with this case.
